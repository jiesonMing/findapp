...
class ApplicationAction extends AdministratorAction
{
    ...
    public function versionByAndroid()
    {
        ...
        $apkUrl = SITE_URL.'/public/ts-Android.apk';
    ...
    {
        if ($_POST) {
            $fileurl = SITE_PATH.'/public/ts-Android.apk';
            if ($_POST['version']) {
                $data['version'] = $_POST['version'];
                $data['is_update'] = $_POST['is_update'];
                $data['title'] = $_POST['title'];
                $data['explain'] = $_POST['explain'];
                model('Xdata')->put('admin_Application:versionByAndroid', $data);
            }
            if ($_FILES['apk']['name']) {
                ...
                unlink($fileurl);
                move_uploaded_file($_FILES['apk']['tmp_name'], $fileurl);
            }
        }
    ...
    {
        $this->pageKeyList = array('version', 'appstore_link', 'is_update', 'title', 'explain');
        array_push($this->pageTab, array(
            'title' => 'Android版本设置',
            'tabHash' => 'versionByAndroid',
            'url' => U('admin/Application/versionByAndroid'),
        ...
        $this->opt['is_update'] = array('0' => '不强制更新', '1' => '强制更新');

...
class Enrollment extends Model
{
    ...
    public function add($uid, $eid, $name, $sex = 0, $num = 1, $phone, $note = null, $time = null)
    {
        ...
        /* 检查报名备注 */
        } elseif (mb_strlen($note, 'UTF8') > 140) {
            $this->error = '报名备注不最多140字';
    ...
    {
        $eid = intval($eid);
        $data = Event::getInstance()->get($eid);
        if ($data['audit'] == 1) {
            $map['aduit'] = '1';
        }
        $map['eid'] = $eid;

        $users = $this->where($map)->field('uid')
          ->select();
        if (!empty($users)) {
     ...
     * @author zsy
     */
    public function getEnrollmentStatus($eid, $uid)
    {
        $event = Event::getInstance()->get($eid);
        if (!$event) {
            $this->error = '操作的活动不存在或者已经被删除';
